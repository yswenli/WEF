...
using WEF.Standard.DevelopTools.JsonTools.CodeWriters;
...
namespace WEF.Standard.DevelopTools.JsonTools
{
    public class JsonClassGenerator : IJsonClassGeneratorConfig
    {
        ...
        public void GenerateClasses()
        {
            ...
            if (ExplicitDeserialization && !(CodeWriter is CSharpCodeWriter)) throw new ArgumentException("只支持C#的反序列化");
            if (used) throw new InvalidOperationException("JsonClassGenerator的此实例已被使用。请创建一个新的实例。");
            used = true;
                    ...
                    foreach (var obj in examples)
                    {
                        ...
                        if (obj.TryGetValue(field.Key, out value))
                        {
                            if (value.Type == JTokenType.Array)
                            {
                                ...
                                {
                                    if (!(item is JObject)) throw new NotSupportedException("尚不支持非对象数组。");
                            ...
                            else if (value.Type == JTokenType.Object)
                            {
                                ...
                                {
                                    if (!(item.Value is JObject)) throw new NotSupportedException("尚不支持非对象数组。");
        ...
        public static readonly string[] FileHeader = new[] { 
            "Power By Leigh.Wen Json Class Generator", 
            "@2015-1-29"
        };
    }
}
